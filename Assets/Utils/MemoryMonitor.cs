...
public class MemoryMonitor : MonoBehaviour
{
	...
	void Update()
	{
		...
		UpdateText(allocatedRamText, "Allocated", allocBytes / BYTES_TO_MB, allocatedColor);
		UpdateText(reservedRamText, "Reserved", resBytes / BYTES_TO_MB, reservedColor);
		UpdateText(monoRamText, "Mono", monoBytes / BYTES_TO_MB, monoColor);
		UpdateText(gcCountText, "GC Count", gcCount, gcHappened ? Color.red : Color.white);

		if (resBytes > maxReserved) 
			maxReserved = resBytes;
	...
	void UpdateText(TextMeshProUGUI text, string type, float value, Color color)
	{
		if (!text) 
			return;
		text.text = $"{type}:\n{value:F2} MB";
